...
module.exports = class LegSON {
            ...
            if (_.isPlainObject(val)) {
                ...
                if (val.toString().length <= this._opts.maxValueLength) {
                    ...
                    if (loader) {
                        ...
                        const eff_filename = path.join(this._opts.basePath, loader.dataPath);
    ...
    load (filename) {
        let rootFilename = path.isAbsolute(filename) ? filename : path.join(this._opts.basePath, filename);

        return async(() => {
            const obj = wait(this._loaders.json(rootFilename));
